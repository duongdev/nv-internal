...
# GPG Encryption Script for Backup Files
# Encrypts or decrypts files using GPG symmetric encryption (AES256)
#
# Usage: ./scripts/backup-encrypt.sh <input-file> [options]
# Options:
#   --decrypt    Decrypt instead of encrypt
#   --output     Custom output path (default: <input>.gpg or <input> without .gpg)
#   --dry-run    Show what would be done without executing
#
# Environment:
#   BACKUP_ENCRYPTION_KEY  Passphrase for encryption/decryption (required)
#
# Examples:
#   ./scripts/backup-encrypt.sh backups/database-2024-01-01-120000.sql.gz
#   ./scripts/backup-encrypt.sh backups/database-2024-01-01-120000.sql.gz.gpg --decrypt
#   ./scripts/backup-encrypt.sh backup.tar --output /tmp/backup.tar.gpg
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -*)
      ...
      echo "Usage: $0 <input-file> [--decrypt] [--output <path>] [--dry-run]"
      exit 1
      ;;
    *)
...
check_env() {
  if [ -z "$BACKUP_ENCRYPTION_KEY" ]; then
    ...
    log_info "Set a strong passphrase for encryption/decryption"
    exit 1
  fi

  # Check key strength (minimum 16 characters recommended)
  local key_length=${#BACKUP_ENCRYPTION_KEY}
  if [ "$key_length" -lt 16 ]; then
    log_warning "Encryption key is shorter than 16 characters (current: $key_length)"
    log_info "Consider using a stronger passphrase"
  else
    log_success "Encryption key is configured (length: $key_length chars)"
  fi
}
...
get_output_path() {
  ...
  if [ "$DECRYPT" = true ]; then
    ...
    if [[ "$INPUT_FILE" == *.gpg ]]; then
      ...
      log_error "Cannot determine output filename for decryption"
...
encrypt_file() {
  ...
  log_info "Cipher: AES256"

  if [ "$DRY_RUN" = true ]; then
    log_warning "DRY RUN - would execute:"
    log_info "  gpg --symmetric --cipher-algo AES256 --batch --yes --passphrase-fd 0 -o \"$output_file\" \"$INPUT_FILE\""
    return
  fi
  ...
  if echo "$BACKUP_ENCRYPTION_KEY" | gpg \
    ...
    echo ""
    echo "ENCRYPTED_FILE=$output_file"
  else
...
main() {
  echo ""
  if [ "$DECRYPT" = true ]; then
    log_info "GPG Decryption Script"
    log_info "====================="
  else
    log_info "GPG Encryption Script"
    log_info "====================="
  fi
  echo ""
